/**
 * Created by Yuqi on 2015/2/9.
 */
var TIMESTAMP = new Date("Mar 29 2004 09:54:18");
...
for (var i = 0; i < logs.length(); i++) {
        if(src[i]._id.date ){
            var temp = {user:"",date:0,src:0,dst:0,diff:0};
            temp.src = src[i].value;
            temp.diff = src[i].value
            loglist[i];
            //printjson(temp);
        }
}
...
logs.forEach(function(log){
    if(log.dateTime){}
        ...
        db.logs.update(log,log.dateTime = new Date(log.TIMESTAMP));
        log.dateTime = new Date(log.TIMESTAMP);
    }
});


db.logs.findAndModify({
